...
def main() -> int:
    ...
    require_equal(manifest.get("scope"), "hermes_managed_release_portable_inputs", "scope")
    ...
    require_equal(manifest.get("required_next_gate"), "DESKTOP-020B3", "next gate")
    ...
    wheelhouse = load_verified_wheelhouse(root / "wheelhouse")
    ...
    require_equal(wheelhouse.locks.get("core"), manifest.get("core_lock_sha256"), "Core lock")
    require_equal(wheelhouse.locks.get("connector"), manifest.get("connector_lock_sha256"), "Connector lock")
    require_equal(
        sha256_file(root / "wheelhouse/WHEELHOUSE-MANIFEST.json"),
    ...
    verification = run_plugin_verifier(runtime_manager, portable_manifest, trust_store, plugin_wheel)
    require_equal(verification.get("signature_verified"), True, "Plugin signature verification")

    proof = load_json(root / "ASSEMBLY-PROOF.json")
    require_equal(proof.get("scope"), "managed_release_offline_assembly_proof", "proof scope")
    require_equal(proof.get("target"), args.expected_target, "proof target")
    require_equal(proof.get("release_id"), manifest.get("release_id"), "proof release_id")
    require_equal(proof.get("portable_plugin_signature_verified"), True, "proof Plugin signature")
    ...
    require_equal(proof.get("network_dependency_install_allowed"), False, "proof network policy")
    require_equal(proof.get("assembled_release_shipped"), False, "proof venv shipping policy")
...
if __name__ == "__main__":
    ...
    except (ValidationError, OSError, ValueError, subprocess.SubprocessError, json.JSONDecodeError) as error:
