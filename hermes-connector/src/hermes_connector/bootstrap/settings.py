...
def load_runtime_settings(
    ...
    if config_reference is not None:
        configured.update(_read_config_file(Path(config_reference)))
    ...
    if any(field not in configured for field in local_plugin_fields):
        raw_hermes_home = source.get("HERMES_HOME")
        if raw_hermes_home is None:
            try:
                raw_hermes_home = os.fspath(
                    Path.home().resolve(strict=True) / ".hermes"
                )
            except (OSError, RuntimeError):
                raise RuntimeConfigurationError(
                    "HERMES_HOME must be canonical"
                ) from None
        ...
        for field, path in local_plugin_defaults.items():
    ...
    credential_store = str(configured.get("credential_store", "keychain"))
    ...
    if credential_store == "keychain":
        ...
        if token_reference is None:
            raise RuntimeConfigurationError(
                "file credential migration mode is incomplete"
            )
        token_file = _absolute_path(token_reference, "token_file")
...
def _read_config_file(path: Path) -> dict[str, object]:
    ...
    _validate_config_metadata(before)
    ...
    try:
        ...
        _validate_config_metadata(opened)
