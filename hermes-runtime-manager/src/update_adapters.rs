...
use crate::release_layout::{
    safe_release_id, validated_console_script, validated_release_directory,
};
...
impl ServiceManagerReleaseActivator {
    ...
    fn release_dir(&self, release_id: &str) -> Result<PathBuf, PortError> {
        validated_release_directory(&self.releases_root, release_id)
    }
    ...
    fn console_script(
        ...
        let release_id = release_dir
            .file_name()
            .and_then(|value| value.to_str())
            .ok_or_else(|| PortError::Operation("release identity is invalid".to_owned()))?;
        validated_console_script(
            &self.releases_root,
            release_id,
            self.platform,
            runtime,
