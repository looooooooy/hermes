...
def _parse_authorization_request(values: dict[str, str]) -> _AuthorizationRequest | None:
    ...
    if (
        ...
        return None
    return _AuthorizationRequest(challenge, redirect_uri, state, provider)
...
</head><body><main><h1>Connect Hermes</h1><p>Sign in to approve this computer. The authorization code can return only to the local Hermes app on this device.</p>{error}
