"""Fail-closed Windows Local Gateway availability."""
...
def test_windows_local_gateway_is_unavailable_and_fails_closed() -> None:
    assert LOCAL_GATEWAY_AVAILABLE is False
    assert LOCAL_GATEWAY_CAPABILITIES.features == frozenset()
    with pytest.raises(
        PlatformLocalGatewayUnavailable,
        match="windows_local_gateway_not_implemented",
    ):
        create_local_gateway_resource(
            settings=object(),
...
def test_windows_local_relays_are_unavailable_and_fail_closed() -> None:
    backend = create_local_relay_backend()

    with pytest.raises(
        PlatformLocalGatewayUnavailable,
        match="windows_local_relay_not_implemented",
    ):
